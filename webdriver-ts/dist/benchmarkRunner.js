...
async function runBenchmakLoopStartup(framework, benchmarkInfo, benchmarkOptions) {
    let warnings = [];
    let errors = [];
    let results = [];
...
async function runBenchmakLoop(framework, benchmarkInfo, benchmarkOptions, 
        ...
        for (let j = 0; j < benchmarkInfos.length; j++) {
            try {
                ...
                if (benchmarkInfos[j].type == BenchmarkType.STARTUP_MAIN) {
                    result = await runBenchmakLoopStartup(runFrameworks[i], benchmarkInfos[j], benchmarkOptions);
                }
