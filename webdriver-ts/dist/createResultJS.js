...
import { benchmarkInfos, BenchmarkType, fileName, slowDownFactor, slowDownNote, warmupNote, } from "./benchmarksCommon.js";
...
async function main() {
            ...
            if (!args.browser || framework.keyed) {
                ...
                if (fs.existsSync(file)) {
                    ...
                    for (let key of Object.keys(data.values)) {
                        ...
                        else if (benchmarkInfo.type === BenchmarkType.STARTUP &&
                            vals.length != config.NUM_ITERATIONS_FOR_BENCHMARK_STARTUP) {
                            console.log(`WARNING: for ${framework.uri} and benchmark ${benchmarkInfo.id} count was ${vals.length}. We expected ${config.NUM_ITERATIONS_FOR_BENCHMARK_STARTUP}`);
                        }
                        else if (benchmarkInfo.type === BenchmarkType.SIZE &&
                            vals.length != config.NUM_ITERATIONS_FOR_BENCHMARK_SIZE) {
                            console.log(`WARNING: for ${framework.uri} and benchmark ${benchmarkInfo.id} count was ${vals.length}. We expected ${config.NUM_ITERATIONS_FOR_BENCHMARK_SIZE}`);
                        }
                    }
                    result.b.push({
    ...
    resultJS +=
        "export const frameworks = " +
            JSON.stringify(frameworks.map((f) => {
                var _a;
                return ({
                    ...
                    frameworkHomeURL: (_a = f.frameworkHomeURL) !== null && _a !== void 0 ? _a : "",
                });
            })) +
            ";\n";
